...
# ===============================
# Fig S1 — Learning curves, 5 clocks x 2 algorithms
# ===============================
library(readr)
library(dplyr)
library(ggplot2)
...
dir_xgb <- "/Users/martalens/Desktop/ML_EAA/output/nov_analyses/XGB/Regressor"
dir_rf  <- "/Users/martalens/Desktop/ML_EAA/output/nov_analyses/RF/Regressor"
out_dir <- "/Users/martalens/Desktop/ML_EAA/output/npj_aging_output/plots"
dir_lasso <- "/Users/martalens/Desktop/ML_EAA/output/nov_analyses/LASSO"

clocks <- c("EAA_DUNEDINMPOA",
            "EAA_GRIMAGE",
            "EAA_LEVINE",
            "EAA_HORVATH",
            "EAA_HANNUM")
...
read_lc_file_lasso <- function(dir_alg, clock_name, algo_label){
    ...
    read_csv(fn, show_col_types = FALSE) |>
        ...
        transmute(
            Clock, Algorithm,
            train_n   = round(train_n_mean),   # ✅ FIX: avoid fractional sizes
...
read_lc_file <- function(dir_alg, clock, algo_label){
    ...
    read_csv(fn, show_col_types = FALSE) |>
        ...
        transmute(
            ...
            train_mean = train_score_mean, train_sd = train_score_sd,
            val_mean   = val_score_mean,   val_sd   = val_score_sd
        )
}
...
# Nice ordering in facets
lc$Clock <- factor(lc$Clock,
                   levels = c("DunedinPoAm","GrimAge","PhenoAge","Horvath","Hannum"))
lc$Algorithm <- factor(lc$Algorithm,
                       levels = c("LASSO", "Random Forest", "XGBoost"))

# ---- long format for Training vs Validation ----
lc_val <- lc |>
    transmute(Clock, Algorithm, train_n,
...
# ---- reference R² (outer-test full model) ----
read_ref <- function(dir_alg, algo_label){
    ...
    df |>
        ...
        distinct(clock, .keep_all = TRUE) |>   # avoid duplicates
        mutate(
            Clock     = recode(clock, !!!clock_labels),
            Algorithm = algo_label
        ) |>
...
out_png <- file.path(out_dir, "Fig_S1_learning_curves_5x2_coloured.png")
